...
class HermesReviewVerifierTests(unittest.TestCase):
    ...
    def test_feishu_runbook_locks_exact_cron_name_id_bindings(self):
        ...
        self.assertIn("record.get('name') == job_name", section)
    ...
    def test_feishu_runbook_report_acceptance_is_schema_safe(self):
        ...
        self.assertNotIn("timedelta", acceptance)
        self.assertNotIn("time(12, 0)", acceptance)
        ...
        self.assertIn("root = Path('/home/ubuntu/workspace/TradingAgents-crypto/results/hermes').resolve(strict=True)", acceptance)
        self.assertIn("stat.S_ISDIR(batch_root.lstat().st_mode)", acceptance)
        ...
        for field_name in ("processed_signal", "final_trade_decision"):
            self.assertRegex(
                acceptance,
                rf"payload\['{field_name}'\] is None or isinstance\(payload\['{field_name}'\], str\)",
            )
    ...
    def test_feishu_runbook_derives_shanghai_date_and_validates_report_schema(self):
        ...
        self.assertNotIn("timedelta", section)
        self.assertNotIn("time(12, 0)", section)
