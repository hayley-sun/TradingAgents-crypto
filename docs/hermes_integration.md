...
部署前，工作树必须干净。将下方占位值替换为已评审、已推送到 `origin` 的 Phase 1 提交 SHA；不得使用未评审分支、强制检出、重置或丢弃本地改动。该流程会分离检出指定提交，不会修改现有 Web `.venv`。
...
`set -e` 确保仅在所有安装和验证成功后才执行 `rm -- "$requirements_file"`；任一步骤失败都会保留每次运行独有的临时文件以便诊断。确认 `git status --short` 无输出后才可继续。操作员必须将 `reviewed_phase1_commit` 占位值替换为已评审并已推送到 `origin` 的提交。
